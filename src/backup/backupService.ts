...
import { BackupOptions, DatabaseType, CloudProvider } from '../type';
import { createZipBackup } from './compress';
import { createCloudStorageProvider } from '../cloud/storageFactory';

const execPromise = promisify(exec);

export async function backupDatabase(options: BackupOptions, progressCallback?: (progress: number) => void): Promise<void> {
...
async function performDatabaseBackup(db: DatabaseType, backupPath: string, progressCallback?: (progress: number) => void): Promise<void> {
...
async function handleCloudUpload(cloud: CloudProvider, backupPath: string, progressCallback?: (progress: number) => void): Promise<void> {
  progressCallback?.(90);
  const cloudStorage = createCloudStorageProvider();
  const cloudProvider = config.cloud[cloud.provider as keyof typeof config.cloud];
...
async function backupPostgres(backupPath: string, progressCallback?: (progress: number) => void): Promise<void> {
...
export async function testDatabaseConnection(options: BackupOptions): Promise<boolean> {
  const { db } = options;
...
async function incrementalBackupPostgres(backupPath: string, lastBackupTimestamp: Date): Promise<void> {
  const { host, user, password, database } = config.db.postgres;
